...
(0, config_1.task)("prove", "Command to generate proof and prove the result of a poll on-chain")
    ...
    const pollContract = await deployment.getContract({ name: types_1.EContracts.Poll, address: pollContracts.poll });
    ...
    const tallyZkey = deployment.getDeployConfigField(types_1.EContracts.VkRegistry, `zkeys.${mode}.tallyVotesZkey`, true);
    const tallyWasm = deployment.getDeployConfigField(types_1.EContracts.VkRegistry, `zkeys.${mode}.tallyWasm`, true);
    const processZkey = deployment.getDeployConfigField(types_1.EContracts.VkRegistry, `zkeys.${mode}.processMessagesZkey`, true);
    const processWasm = deployment.getDeployConfigField(types_1.EContracts.VkRegistry, `zkeys.${mode}.processWasm`, true);
